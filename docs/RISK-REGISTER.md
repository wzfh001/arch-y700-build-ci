...
| R-004 | P0 | Generic WCN7850 firmware wins | Wi-Fi unavailable | Exact hashes, native package, independent path | Final-raw ownership/content audit | SOURCE CONTROL VERIFIED by SRC-20260722-005; FINAL RAW/HARDWARE UNTESTED |
| R-005 | P1 | USB service waits forever | Delayed/blocked boot | Persistent non-blocking coordinator | Journal + command-timeout tests | SOURCE CONTROL VERIFIED; HARDWARE UNTESTED |
...
| R-009 | P1 | Rolling update breaks payload ABI | Boot/hardware regression | Immutable pacman DB/package lock plus frozen compatibility unit | Roll back known image/config snapshot | LOCK VERIFIED by AUDIT-20260722-002; UPDATE ROLLBACK UNTESTED |
...
| R-011 | P3 | Provider query is mistaken for an exact installed package | Valid replacement build stops or profile audit lies | Match exact names from the complete `pacman -Qq` list | Provider and near-match regressions; final-raw package audit | SOURCE CONTROL VERIFIED by SRC-20260722-015; FINAL RAW UNTESTED |
